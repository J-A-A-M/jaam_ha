...
- **Repository:** J-A-A-M/jaam_ha

**Key directories:**

- `custom_components/jaam_ha/` - Main integration code
...
- Integration logs: `custom_components.jaam_ha: debug` in `config/configuration.yaml`
...
- Use the domain `jaam_ha` for all DOMAIN references
- Prefix all integration-specific classes with `JaamHA`
- Use "JAAM Home Assistant Integration" as the display title
...
- Inherit from platform base + `JaamHAEntity`
...
- `tests/` mirrors `custom_components/jaam_ha/` structure
