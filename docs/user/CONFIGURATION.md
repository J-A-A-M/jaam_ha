...
This document describes all configuration options and settings available in the JAAM Home Assistant Integration custom integration.
...
```yaml
service: jaam_ha.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: jaam_ha.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: JAAM Home Assistant Integration Alert
...
- [GitHub Issues](https://github.com/J-A-A-M/jaam_ha/issues) - Report problems
