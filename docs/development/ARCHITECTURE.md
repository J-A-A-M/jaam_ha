...
This document describes the technical architecture of the JAAM Home Assistant Integration custom component for Home Assistant.

## Directory Structure

```text
custom_components/jaam_ha/
...
│   ├── __init__.py          # Exports JaamHADataUpdateCoordinator
...
│   ├── __init__.py          # Exports JaamHAEntity
...
- `base.py` - Main coordinator class (`JaamHADataUpdateCoordinator`)
...
**Key class:** `JaamHADataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `JaamHAConfigFlowHandler` (main flow)
- `JaamHAOptionsFlow` (options)
...
**Key class:** `JaamHAEntity` (in `entity/base.py`)
...
2. `JaamHAEntity` for common functionality
...
1. Create directory: `custom_components/jaam_ha/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `JaamHAEntity`
