...
- **`entity/`** - Base entity classes (inherit `JaamHAEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, JaamHAEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import JaamHAEntity`
